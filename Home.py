...
if check_password():
   ...
   if select_cat == "Договір на послуги з ремонту тепловозу":
      ...
      with st.expander("Зазначте необхідні дані для завершення формування типового договору"):
         ...
         input02 = st.text_input("Період (рік) проведення ремонтів", [2022, 2023])
      ...
      with use_col:
         use_draft = st.download_button("Завантажити", key='use_draft', data=file)
      
      if use_draft:
         st.write("Проєкт договору потребує погодження...")

      if st.session_state.make_contract:
         st.write("""Ви можете сформувати проєкт договору на основі наявних шаблонів, 
         що прискорить процедуру його погодження""")
         ...
         st.multiselect("Використати шаблони", options=chapters, default=chapters)